...
to  docs/row_regions_tuned.json.  The sniper and preview tools load this
file automatically when present.

Usage:
    python tools/tune_rows.py
"""
...
# ── Formula defaults (kept in sync with preview_rows.py) ─────────────────────
...
ROW_COLORS = [
    QColor(255, 60, 60, 220),  # red
    QColor(60, 210, 60, 220),  # green
    QColor(60, 150, 255, 220),  # blue
    QColor(220, 120, 255, 220),  # purple
]
SEL_ALPHA = 255  # selected row border opacity
...
def _base_rows(win_x: int, win_y: int, win_w: int, win_h: int) -> list[tuple[int, int, int, int]]:
    """Formula row positions in Qt logical pixels — matches preview_rows.py exactly.

    Only X uses DPR correction.  Y / width / height use the raw physical
    pygetwindow values directly as Qt logical coordinates; the formula
    constants absorb the DPR factor for those axes (same arithmetic trick
    that preview_rows.py relies on).
    """
    dpr = _dpr()
...
def _load_tuned(win_w: int | None = None, win_h: int | None = None) -> list[dict] | None:
    """Load per-row tuning for the given window size, if any.

    Supports both legacy flat format and multi-profile format.
    When win_w/win_h are given, picks the closest matching profile via
    window_utils.best_matching_row_profile() using logical dimensions.
    """
    path = os.path.join(
...
class TuneOverlay(QWidget):
    ...
    def paintEvent(self, _event) -> None:
        ...
        for i, (rx, ry, rw, rh) in enumerate(self._rows):
            ...
            lc.setAlpha(255)
            painter.setPen(lc)
            label = f"▶ Row {i + 1}" if is_sel else f"Row {i + 1}"
...
class ControlPanel(QWidget):
    def __init__(self, overlay: TuneOverlay) -> None:
        ...
        self._sel = 0
        # Per-row pixel adjustments from formula baseline (in Qt logical px)
        self._adjs: list[list[int]] = [[0, 0, 0, 0] for _ in range(NUM_ROWS)]
        self._base: list[tuple[int, int, int, int]] = []
        self._win_phys: tuple[int, int, int, int] | None = None  # left,top,width,height (physical)
        self._badge: dict | None = None  # loaded lazily on first refresh with live window dims
    ...
    def _try_load_tuned(self) -> None:
        """Schedule tuning load on the next refresh (deferred until we have live window dims)."""
        self._pending_tuned = True  # sentinel — actual load happens in _refresh() with ww/wh
    ...
    def _adj(self, dx: int = 0, dy: int = 0, dw: int = 0, dh: int = 0) -> None:
        ...
        a[1] += dy
        a[2] += dw
        # Clamp height: never let a row become less than 10 px tall
        if self._base:
    ...
    def _refresh(self) -> None:
        ...
        self.win_label.setText(
            f"FH6: {win.width}×{win.height} phys  ({ww_l}×{wh_l} logical)  DPR={dpr:.2f}"
        )

        self._base = _base_rows(win.left, win.top, win.width, win.height)
        self._win_phys = (win.left, win.top, win.width, win.height)

        # Load badge params on first window read so we can pick the right profile
        if not self._badge_loaded:
            self._badge = _load_badge_params(win.width, win.height)
            self._badge_loaded = True

        # Apply saved tuning on first window read — load with live dims for profile selection
        if hasattr(self, "_pending_tuned") and self._win_phys:
            wx, wy, ww, wh = self._win_phys  # physical
            rows_to_apply = _load_tuned(ww, wh)
            if rows_to_apply:
                for i, row_pct in enumerate(rows_to_apply[:NUM_ROWS]):
                    bx, by, bw, bh = self._base[i]
                    tx = int(wx / dpr + (ww / dpr) * row_pct["x_pct"])  # x: logical-ref
                    ty = wy + int(wh * row_pct["y_pct"])  # y: physical-ref
                    tw = int(ww * row_pct["w_pct"])  # w: physical-ref
                    th = int(wh * row_pct["h_pct"])  # h: physical-ref
        ...
        # Update info label for selected row
        if rows:
            rx, ry, rw, rh = rows[self._sel]
            bx, by, bw, bh = self._base[self._sel]
            wx, wy, ww, wh = self._win_phys
            dx, dy, dw, dh = self._adjs[self._sel]
            x_pct = (rx - wx / dpr) / (ww / dpr) if ww else 0  # logical-ref
            y_pct = (ry - wy) / wh if wh else 0  # physical-ref
            w_pct = rw / ww if ww else 0  # physical-ref
            h_pct = rh / wh if wh else 0  # physical-ref
            self.row_info.setText(
                f"Row {self._sel + 1}:  ({rx},{ry})  {rw}×{rh} px\n"
    ...
    def _save(self) -> None:
        ...
        for rx, ry, rw, rh in rows:
            # x_pct: logical-ref fraction — reconstructed as wx/dpr + (ww/dpr)*x_pct
            # y/w/h_pct: physical-ref fractions — reconstructed as wy + wh*y_pct, etc.
            # get_tuned_row_regions() applies the same arithmetic and arrives at the
            # correct pyautogui physical coords (see docs/sold-detection.md).
            row_data.append(
                {
        ...
        # Load existing data and migrate to multi-profile format if needed
        try:
        ...
        # Replace an existing profile within 5% of current window size, or append new
        new_profile: dict = {
            "label": f"{ww}x{wh}",
        ...
        self.status_label.setText(
            f"Saved → docs/row_regions_tuned.json  ({len(profiles)} profile(s))\n"
            + "\n".join(
